 ...
 * @property int $count_adult Количество взрослых пассажиров
 * @property string $date_begin2 Дата и время начала обратного трансфера
 * @property int $count_avtolulka Количество детей(до 10 кг, до 6 месяцев)
 * @property int $count_avtokreslo Количество детей(9–25 кг, 0–7 лет)
 * @property int $count_buster Количество детей(22–36 кг, 6–12 лет)
 * @property int $return Добавить обратный маршрут
 ...
 * @property string $classification Классификация
...
class Tasks extends \yii\db\ActiveRecord
{
   ...
   public function rules()
    ...
    public function getItemsList()
    {
        ...
        {
            $data = ItemsDescription::find()->all();
            return ArrayHelper::map($data,'id','name');
        }else{
            $data = \backend\models\Translates::find()->where(['table_name'=>'items_description','language_code'=>Yii::$app->language])->all();
            return ArrayHelper::map($data,'field_id','field_value');
        }
   ...
   { 
       return $this->hasMany(TaskItems::className(), ['task_id' => 'id']); 
   } 
