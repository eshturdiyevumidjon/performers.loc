...
/*    var st_head_fixed;
    $.post('/site/st-head-fixed', { }, function(data){ st_head_fixed = data; });    

    var st_sb_fixed;
    $.post('/site/st-sb-fixed', { }, function(data){ st_sb_fixed = data; });

    var st_sb_scroll;
    $.post('/site/st-sb-scroll', { }, function(data){ st_sb_scroll = data; });

    var st_sb_right;
    $.post('/site/st-sb-right', { }, function(data){ st_sb_right = data; });

    var st_sb_custom;
    $.post('/site/st-sb-custom', { }, function(data){ st_sb_custom = data; });

    var st_sb_toggled;
    $.post('/site/st-sb-toggled', { }, function(data){ st_sb_toggled = data; });

    var st_layout_boxed;
    $.post('/site/st-layout-boxed', { }, function(data){ st_layout_boxed = data; });*/

    /* Default settings */
    var theme_settings = {
        st_head_fixed: $('#st_head_fixed').val() * 1,
        st_sb_fixed: $('#st_sb_fixed').val() * 1,
        st_sb_scroll: $('#st_sb_scroll').val() * 1,
        st_sb_right: $('#st_sb_right').val() * 1,
        st_sb_custom: $('#st_sb_custom').val() * 1,
        st_sb_toggled: $('#st_sb_toggled').val() * 1,
        st_layout_boxed: $('#st_layout_boxed').val() * 1
    };
    /*var theme_settings = {
        st_head_fixed: 0,
        st_sb_fixed: 1,
    ...
    $(".theme-settings input").on("ifClicked",function(){
        ...
        if(input.attr("name") === 'st_sb_fixed'){
            if(theme_settings.st_sb_fixed == 1){
                ...
                //$.post('/site/set-theme-values', { st_sb_scroll : 0 }, function(data){ });
            }
        }
        ...
        if(input.attr("name") === 'st_sb_scroll'){
            if(theme_settings.st_sb_scroll == 1 && theme_settings.st_layout_boxed == 0){
                ...
                //$.post('/site/set-theme-values', { st_sb_fixed : 0 }, function(data){ });
            }
        }
